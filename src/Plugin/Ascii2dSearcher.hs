...
getAscii2dUrl imgUrl = do
  ...
  let colorUrl = fixUrl.("/search/color"<>) <$> searchBetweenBL "/search/color" "\">" rContent
  let bovwUrl  = fixUrl.("/search/bovw"<>) <$> searchBetweenBL "/search/bovw" "\">" rContent
...
processAscii2dSearch (_, update) =
   case message_image_urls update of
     ...
     _ -> pure [makeReqFromUpdate update a2dHelps]

a2dHelps :: Text.Text
a2dHelps = Text.unlines [ "====Ascii2dSearcher===="
                        , "/asc<PIC>: 从ascii2d.net搜索一张图片。"
                        ]
