...
getNHentaiBookId bName = do
  ...
  pure $ either'
    ...
    (\nh -> maybe'
      (nh_result nh)
      Nothing
      (\results -> if null results
        then Nothing
        else Just $ maybe' (find (elem "chinese" . nh_tags) results)
                           (getInfo $ head results)
                           getInfo
      )
    )
  where getInfo x = (,) (nh_title x) (nh_id x)

processNHentaiQuery :: (Text.Text, Update) -> IO [SendMsg]
processNHentaiQuery (cmdBody, update) = if content /= ""
  then do
    logWT Info
      $  "NHentai query: ["
      <> Text.unpack content
    ...
    case r of
      Just result ->
        let msg = Misc.unlines
              [ "[标题] " <> fst result
              , "[链接] https://nhentai.net/g/" <> snd result
              ]
        in  pure [makeReqFromUpdate update msg]
...
instance FromJSON NHentaiResult where
  parseJSON = withObject "NHentaiResult" $ \v ->
    NHentaiResult
      <$> ((v .: "id") >>= parseId)
      <*> ((v .: "title") >>= (.: "japanese"))
      <*> (   (v .: "tags" :: Parser Array)
