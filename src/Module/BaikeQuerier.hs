...
getWords ""  = []
getWords str = (strip . toStrict . decodeUtf8 $ fst (breakOn "<" xs))
  : getWords xs
...
runBaiduSearch query = do
  x <- runMEitherT $ do
    realUrl <- liftMaybe
      "未找到词条。"
      (getFstUrl <$> getWith opts "https://www.baidu.com/s")
    realRst <- liftMaybe ("词条无摘要:\n" <> Text.pack realUrl)
                         (getFirstPara <$> get realUrl)
...
processBaiduQuery (cmdBody, update) = if cmdBody /= ""
  then do
    result <- runBaiduSearch cmdBody
    logWT Info $ "Query: [" <> unpack cmdBody <> "] sending from " <> show
      (user_id update)
