...
getSolidotContent = do
  ...
  pure $ splitEveryThree $ drop 1 $ searchAllBetweenBL "CDATA["
                                                       "]]"
                                                       (r ^. responseBody)
...
addSubscriber (_, update) = do
  ...
  if snd (T.breakOn (T.pack $ show chatId) ss) == ""
    ...
    else pure [makeReqFromUpdate update "您已在订阅列表内。"]
...
checkNewOfSolidot = do
  ...
  if snd
         (T.breakOn (TL.toStrict . decodeUtf8 . get1st $ head newContent)
                    originContent
         )
       == ""
    then do
      BL.writeFile (head sfRqmt)
        $   mconcat
        .   intersperse "\n"
        $   get1st
        <$> newContent
      ...
      pure
        ...
        <$> takeWhile
              (\(title, _, _) ->
                snd (T.breakOn ((TL.toStrict . decodeUtf8) title) originContent)
                  == ""
              )
              newContent
    else pure []
