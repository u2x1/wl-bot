...
toTGChannelSM x =
  SendMsg "@hostloc_feed" "0" Private Telegram Nothing Nothing Nothing (Just x)

trans2SendMsg :: Thread -> IO T.Text
trans2SendMsg Thread { tid = t', name = n, time = t, reply = r', view = v } =
  do
    let link = "https://www.hostloc.com/thread-" <> show t' <> "-1-1.html"
        archv_link =
          "https://www.hostloc.com/archiver/tid-" <> show t' <> ".html"
...
getPreview link = do
  ...
  let rst      = fromMaybe "" $ searchBetweenBL "</h3" "\t\t\t" rsp
      realText = filter
        (not . BL.null)
        (   BL.filter (\x -> x /= 13 && x /= 10 && x /= 9)
        <$> searchAllBetweenBL ">" "<" rst
        )
      retText = if BL.length (mconcat realText) > 300
        then
          let x = Misc.unlines (Prelude.take 4 realText)
          in  if BL.length x > 500 then BL.take 500 x else x
...
thread = do
  tid' <-
    str2Int
    .   toString
    <$> (manyTill anyWord8 (string "tid=") *> takeTill (== 38))
  name' <-
    toString
    .   BS.pack
    <$> (  manyTill anyWord8 (string "class=\"s xst\">")
        *> manyTill anyWord8 (string "</a>")
        )
  time' <-
    timestr2timeint
    .   toString
    <$> (  manyTill anyWord8 (string "xi1\"><")
        *> takeTill (== 62)
        *> word8 62
        *> takeTill (== 60)
        )
  reply' <-
    ...
    .   toString
    .   BS.pack
    <$> (manyTill anyWord8 (string "xi2\">") *> manyTill anyWord8 (word8 60))
  view' <-
    str2Int
    .   toString
    .   BS.pack
    <$> (string "/a><em>" *> manyTill anyWord8 (word8 60))
  return $ Thread tid' name' time' view' reply'

timestr2timeint :: String -> Int
timestr2timeint str = if null timeStr then 30 else str2Int timeStr * timeScale
 where
...
checkExist conn th@Thread { tid = tid' } = do
  ...
  where qr = "SELECT * FROM hostloc WHERE tid = :tid"

writeHotThs :: (Traversable t, ToRow q) => Connection -> t q -> IO (t ())
writeHotThs conn =
  traverse (execute conn "INSERT OR IGNORE INTO hostloc VALUES (?, ?)")
