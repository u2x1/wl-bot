...
getTrtLinks num keyword = do
  ...
  let links =
        Misc.searchAllBetweenBL "/information/" "\""
          $  BL.drop 18000
          $  r
          ^. responseBody
  ...
  if null linksTen
    then pure Nothing
      ...
      .   Just
      $   toString
      .   ("https://www.torrentkitty.tv/information/" <>)
      <$> linksTen
  ...
  traverse_
    (\url -> forkIO $ do
      ...
      let rawHtml = r ^. responseBody
          ...
          tMag = toStrict . decodeUtf8 . ("magnet:?xt=" <>) <$> searchBetweenBL
            "magnet:?xt="
            "<"
            (r ^. responseBody)
      writeChan ch $ TrtInfo <$> tName <*> tDate <*> pure tFiles <*> tMag
    )
    links
  catMaybes <$> replicateM (length links) (readChan ch)

generateTrtTexts :: [TrtInfo] -> IO [[DrawText]]
generateTrtTexts trtInfos = pure $ addInfo . mconcat . addNumber $ fmap
  concatTrtInfo
  trtInfos
 where
  ...
  go cnt (x : xs) =
    modifyHead x (("[" <> show cnt <> "] ") <>) : go (cnt + 1) xs
  go _ [] = []
...
concatTrtInfo info =
 ...
 where
  notUselessInfo (y, x) =
    snd (BL.breakAfter "BitComet" y)
      == ""
      && (snd (BL.breakAfter "G" x) /= "" || snd (BL.breakAfter "M" x) /= "")
  hLongText x =
    let len = BL.length x
    in  if len > 170 then BL.take 160 x <> " ... " <> BL.drop (len - 5) x else x
  addIndent = map
    (\(a, b) ->
      [ DrawText (tab <> "|- " <> toString (hLongText a) <> " ") defaultFont
      , DrawText (toString b) redFont
      ]
    )
...
processTrtQurey (cmdBody, update) = do
  ...
  case length mode of
    ...
    1 -> if cmdBody == "!clear"
      then do
        removeDirectoryRecursive "wldata/Torrents"
          >> createDirectory "wldata/Torrents"
          ...
          then do
            ...
            case links' of
              Just links -> do
                ...
                BL.writeFile
                    ("wldata/Torrents/TR-" <> Text.unpack cmdBody <> ".txt")
                  $ encode
                  $ fmap trt_mag infos
              _ -> pure ()
          else pure ()
    ...
    2 -> do
      ...
      if i > defaultQueryCnt
        then pure [makeReqFromUpdate update "[错误] 索引超出界限。"]
          ...
          let magCachePath =
                "wldata/Torrents/TR-" <> Text.unpack (head mode) <> ".txt"
          exist <- doesFileExist magCachePath
          if not exist
            then pure [makeReqFromUpdate update "[错误] 本地无缓存。"]
            else do
              mags <-
                eitherDecode <$> BL.readFile magCachePath :: IO
                  (Either String Array)
              case mags of
                Left err ->
                  pure [makeReqFromUpdate update $ "[错误] " <> Text.pack err]
                Right lst ->
                  pure [makeReqFromUpdate update $ "[磁链] " <> showStr (lst ! i)]
