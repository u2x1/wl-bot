...
runWAITSearch imgUrl' = do
  ...
  r <-
    try $ post "https://trace.moe/api/search" ["image" := imgBase64] :: IO
...
processWAITQuery (_, update) =
  let imgUrls = message_image_urls update
  in
    if null imgUrls
      then pure [makeReqFromUpdate update "无效图片。"]
      else do
        result <- runWAITSearch $ head imgUrls
        logWT Info $ "WAIT [" <> head imgUrls <> "] sending from " <> show
          (user_id update)
        either'
          result
          (\x -> pure [makeReqFromUpdate update $ Text.pack x])
          (\rst -> pure
 ...
 where
  toTime x = Text.pack $ show (div (round x) 60 :: Int) <> ":" <> show
    (mod (round x) 60 :: Int)
...
instance FromJSON WAITResults where
  parseJSON = withObject "WAITResults" $ \v -> WAITResults <$> (v .: "docs")
...
instance FromJSON WAITResult where
  parseJSON = withObject "WAITResults" $ \v ->
    WAITResult
      <$> ((* 100) <$> v .: "similarity")
      <*> (v .: "anime")
      <*> (v .: "season")
