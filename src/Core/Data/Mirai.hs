...
import           Core.Type.Mirai.Update  as MU (MRMsg, mrm_text, mrm_type,
                                                mrm_url)
import           Core.Type.Unity.Request as UR (SendMsg, imgPath, imgUrl, text)
...
getImgUrls cqMsgs = catMaybes (mrm_url <$> filter (\m -> mrm_type m == "Image") cqMsgs)
